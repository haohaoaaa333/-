...
6. 进入“草稿箱”，按待审、已通过、已驳回或 Gemini 风险题筛选。每页显示 10 题。
7. 左栏核对原始 OCR 文字与图片，中栏修改模块、题干、A-D、材料、答案和解析，右栏查看 Gemini 建议。
8. 点击“保存修正”后再决定通过或驳回。图形题包含合成选项图时，必须勾选“已查看合成图并确认 A-D 顺序”。
9. 可逐题调用 Gemini，也可审核整套；含图题会强制保留人工复核，Gemini 不会自动替代看图确认。
...
Gemini 密钥只配置在云函数环境变量 `GEMINI_API_KEY` 中，不写入浏览器、本地存储、源码或题库文件。可选配置：
