...
function allowedRolesForAction(action) {
  ...
  if (/^import_task\.(preflight|create|cancel|retry|log|recover|resplit)$/.test(action)) return ['super_admin', 'editor'];
  ...
  if (/^file\.delete_temp$/.test(action)) return ['super_admin', 'editor'];
